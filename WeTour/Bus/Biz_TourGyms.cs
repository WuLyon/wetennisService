...
namespace WeTour
{
    ...
    public class Biz_TourGyms
    {
        ...
        #region Lyon
        public List<Model_TourGymCourts> GetTourGymCourtsByTourSys(string _TourSys)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("select * from Tour_GymCourts where TourSys='" + _TourSys + "'");
            DataTable dt = DbHelperSQL.Query(sb.ToString()).Tables[0];
            List<Model_TourGymCourts> list = new List<Model_TourGymCourts>();
            if (dt.Rows.Count > 0)
            {
                list = WeTour.JsonHelper.ParseDtModelList<List<Model_TourGymCourts>>(dt);
            }
            return list;
        }
        #endregion
